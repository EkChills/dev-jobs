...
export default function JobCard({postedAt, contract, position, location, logo, company, logoBackground, id}:Job) {
  ...
  return (
    <motion.div  className='flex flex-col rounded-md px-8 pb-8 pt-[49px] bg-white dark:bg-[#19202D] cursor-pointer relative -z-1 hover:shadow-xl transition-shadow duration-500' initial={{opacity:0}} whileInView={{opacity:1}} onClick={() => router.push(`${baseUrl}/jobs/${id}`)} >
